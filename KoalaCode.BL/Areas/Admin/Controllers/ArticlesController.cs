...
namespace KoalaCode.BL.Areas.Admin.Controllers
{
    public class ArticlesController : BaseAuthRequired
    {
        ...
        {
            return PartialView("_UserArticlesList", GetListUserArticlesModel());
        }
        ...
        private List<ListUserArticlesModel> GetListUserArticlesModel()
        {
            var model = new List<ListUserArticlesModel>();
            var user = UnitOfWork.Users.GetById(UserData.GetUserInfo().Id);
            var articles = UnitOfWork.Article.GetArticlesByUser(user);
            
            if (articles == null) return model;

            model.AddRange(articles.Select(a => new ListUserArticlesModel
            {
                Id = a.Id, Headline = a.Headline, ShortDescription = a.ShortDescription, CreatedDate = a.CreatedDate
            }));
